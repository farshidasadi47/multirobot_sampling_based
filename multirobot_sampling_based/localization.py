...
class Localization:
    ...
    def __init__(self, raw=False):
        ...
        package_dir1 = os.path.join(r"/ros_ws/src/", package_name)
        package_dir2 = os.path.join(os.getenv("HOME", ""), package_name)
        ...
        self._img_dir_prefix = os.path.join(
            package_dir, package_name, "calibration_img"
        )
        self._mtx = None
        self._dist = None
        self._nmtx = None
    ...
    def _set_hsv_ranges(self):
        ...
        if self._fps == 60:
            ...
            # Black border
            border_hsv_ranges = {
                "lb": [np.array([20, 20, 20], dtype=np.uint8)],
                "ub": [np.array([179, 255, 130], dtype=np.uint8)],
            }
            # Black or 'k'
            hsv_ranges["k"] = {
                "lb": [np.array([20, 20, 20], dtype=np.uint8)],
            ...
            hsv_ranges["r"] = {
                "lb": [
                    np.array([0, 70, 100], dtype=np.uint8),
                    np.array([175, 20, 100], dtype=np.uint8),
                ],
                "ub": [
            ...
            # Blue or 'b'
            hsv_ranges["b"] = {
                "lb": [np.array([90, 50, 100], dtype=np.uint8)],
                "ub": [np.array([120, 255, 240], dtype=np.uint8)],
            }
            # Green or 'g'
            hsv_ranges["lime"] = {
                "lb": [np.array([40, 50, 140], dtype=np.uint8)],
                "ub": [np.array([80, 255, 255], dtype=np.uint8)],
            }
            # White or 'm'
            hsv_ranges["fuchsia"] = {
                "lb": [np.array([155, 70, 100], dtype=np.uint8)],
                "ub": [np.array([179, 255, 255], dtype=np.uint8)],
            }
        #
